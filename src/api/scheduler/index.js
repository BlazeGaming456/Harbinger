...
async function scheduleTick() {
  const result = await pool.query(
    `SELECT id, url FROM endpoints WHERE is_active = true AND next_probe_at <= now()`,
  );

  for (const endpoint of result.rows) {
    await probeQueue.add("probe", {
      endpointId: endpoint.id,
      url: endpoint.url,
    });
    await pool.query(
      `UPDATE endpoints SET next_probe_at = now() + (interval_seconds || ' seconds')::interval WHERE id = $1`,
      [endpoint.id],
    );
  }
