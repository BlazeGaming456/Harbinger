...
function aggregate(probes) {
  ...
  return {
    ...
    errors:
      probes.filter((probe) => probe.status_code >= 400 || !probe.status_code)
...
function score(metrics) {
  return Math.min(
    (metrics.p95 / 1500) * 0.3 + metrics.errors * 0.4 + metrics.timeouts * 0.3,
    1,
  );
}
...
new Worker(
  ...
  async (job) => {
    ...
    const { rows } = await pool.query(
      "SELECT * FROM probe_results WHERE endpoint_id = $1 ORDER BY probed_at DESC LIMIT 40",
      [endpointId],
    );
    ...
    await pool.query(
      `INSERT INTO endpoint_scores (endpoint_id, score, p95_latency_ms, error_rate, timeout_rate, computed_at) VALUES ($1, $2, $3, $4, $5, now()) ON CONFLICT (endpoint_id) DO UPDATE SET score = $2, p95_latency_ms = $3, error_rate = $4, timeout_rate = $5, computed_at = now()`,
      [
        endpointId,
        recentScore,
        Math.round(recent.p95),
        recent.errors,
        recent.timeouts,
      ],
    );
    ...
    await redis.publish(
      "score-updates",
      JSON.stringify({
        userId: endpoint.rows[0].user_id,
        endpointId,
        score: recentScore,
        p95_latency_ms: recent.p95,
    ...
    if (recentScore > 0.7) {
      const existing = await pool.query(
        `SELECT id, alert_id, opened_at, reminder_sent FROM incidents WHERE endpoint_id = $1 AND incident_type = 'degradation' AND resolved_at IS NULL LIMIT 1`,
        [endpointId],
      );
      if (!existing.rows[0]) {
        ...
        Date.now() - new Date(existing.rows[0].opened_at).getTime() >= 86400000
      ) {
        await alertQueue.add("alert", {
      ...
      await pool.query(
        "UPDATE incidents SET resolved_at = now() WHERE endpoint_id = $1 AND resolved_at IS NULL",
        [endpointId],
      );
    }
    ...
    if (trend > 0.15 && recentScore <= 0.7) {
      const earlyWarning = await pool.query(
        `INSERT INTO incidents (endpoint_id, incident_type) VALUES ($1, 'early_warning') ON CONFLICT DO NOTHING RETURNING id, alert_id`,
        [endpointId],
      );
      if (earlyWarning.rows[0]) {
        await alertQueue.add("alert", {
          incidentId: earlyWarning.rows[0].id,
          alertId: earlyWarning.rows[0].alert_id,
          endpointId,
          traceId,
      ...
      await pool.query(
        `UPDATE incidents SET resolved_at = now() WHERE endpoint_id = $1 AND incident_type = 'early_warning' AND resolved_at IS NULL`,
        [endpointId],
      );
    }
  },
...
new Worker(
  ...
  async (job) => {
    ...
    if (channels.includes("email") || channels.includes("all")) {
      await CHANNELS.email(user.alert_target || user.email, payload);
      delivered = true;
    }
    if (
      ...
      const target = user.webhook_url || user.alert_target;
      if (target?.startsWith("http")) {
        await CHANNELS.webhook(target, payload);
        delivered = true;
      }
    }
    if (delivered)
      await pool.query(
        `UPDATE incidents SET ${alertKind === "degradation_reminder" ? "reminder_sent" : "alert_sent"} = true WHERE id = $1`,
        [incidentId],
      );
