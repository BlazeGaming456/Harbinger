...
export async function userRoutes(app) {
    app.get('/users/me', { preHandler: requireAuth }, async (req, reply) => {
        ...
        return {
            ...user,
            email_alerts_enabled: channels.includes('email') || channels.includes('all'),
            webhook_alerts_enabled: channels.includes('webhook') || channels.includes('slack') || channels.includes('all'),
        };
    });

    app.patch('/users/me', { preHandler: requireAuth }, async (req, reply) => {
        const { email_alerts_enabled, webhook_alerts_enabled, alert_target, webhook_url, alert_channel } = req.body || {};

        let activeChannels = [];
        if (email_alerts_enabled !== undefined || webhook_alerts_enabled !== undefined) {
        ...
        const newChannelStr = activeChannels.length > 0 ? activeChannels.join(',') : 'none';
        ...
        await pool.query(
            `UPDATE users 
             ...
             WHERE id = $4`,
            [newChannelStr, alert_target !== undefined ? alert_target : null, webhook_url !== undefined ? webhook_url : null, req.userId]
        );
        ...
        const user = result.rows[0];
        const channels = (user.alert_channel || 'email').split(',').map(c => c.trim().toLowerCase());
