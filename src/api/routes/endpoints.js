...
export async function endpointRoutes(app) {
      ...
      try {
        ...
        for (const endpoint of endpoints) {
          const result = await client.query(
            `INSERT INTO endpoints (user_id, url, name, interval_seconds, next_probe_at)
                     VALUES ($1, $2, $3, $4, now()) RETURNING *`,
            [
              req.userId,
              endpoint.url,
              endpoint.name || null,
              endpoint.interval_seconds || 60,
            ],
          );
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const nextCursor =
        rows.length === capped ? rows[rows.length - 1].created_at : null;
